...
#~~~~~~~~~~~~~~~~~
# To move from previous version to v0.9.2-RC1
# MATVIEW Create User list linked to RADIUS name for Freeradius plugin
# Matview name : list_user_freeradius
# Old format : None
# New format : [{"radiusname": "Radius IRIS Recette", "username": "toto"}, {"radiusname": "Radius IRIS Recette", "username": "tutu"}]
#~~~~~~~~~~~~~~~~~

import json
import psycopg2

# INSERT INTO matview (name, view_data) VALUES ('list_user_freeradius', '[{"ip": "toto", "radiusname": "Radius IRIS Recette"}, {"ip": "147.17.1.254", "radiusname": "Radius IRIS Recette"}, {"ip": "147.17.1.252", "radiusname": "Radius IRIS Recette"}, {"ip": "tutu", "radiusname": "Radius IRIS Recette"}]')'
...
d_search_options['filter'] = 'name = \'list_user_freeradius\''

l_results = IMDB.search(collection='matview', search_parameter=d_search_options)

d_search_options['fields'] = 'json_build_object(\'username\', user_info->>\'username\', \'radiusname\', user_info->>\'radiusname\')'
d_search_options['filter'] = 'all'
l_user = IMDB.search(collection='users_freeradius', search_parameter=d_search_options)
	
for user in l_user :
	username = user[0]['username']
	radiusname = user[0]['radiusname']
	l_user_to_view.append({'radiusname': radiusname, 'username': username})

to_insert = json.dumps(l_user_to_view).replace('\'', '\'\'')
