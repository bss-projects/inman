...
# MATVIEW Add RADIUS name into Client List for Freeradius plugin
# Matview name : list_client_freeradius
# Old format : ["147.17.1.252", "192.168.42.45"]
# New format : [{"radiusname": "Radius IRIS Recette", "ip": "147.17.1.252"}, {"radiusname": "Radius IRIS Recette", "ip": "192.168.42.45"}]
#~~~~~~~~~~~~~~~~~

import json
import psycopg2

# UPDATE matview SET (view_data) = ('[{"ip": "192.168.42.45", "radiusname": "Radius IRIS Recette"}, {"ip": "147.17.1.254", "radiusname": "Radius IRIS Recette"}, {"ip": "147.17.1.252", "radiusname": "Radius IRIS Recette"}, {"ip": "147.17.1.250", "radiusname": "Radius IRIS Recette"}]') WHERE name = 'list_client_freeradius'
...
d_search_options['fields'] = 'json_build_object(\'ip\', client_info->>\'ip\', \'radiusname\', client_info->>\'radiusname\')'
d_search_options['filter'] = 'all'
l_client = IMDB.search(collection='client_freeradius', search_parameter=d_search_options)
	
for client in l_client :
	ip = client[0]['ip']
	radiusname = client[0]['radiusname']
	l_client_to_view.append({'radiusname': radiusname, 'ip': ip})

to_insert = json.dumps(l_client_to_view).replace('\'', '\'\'')
