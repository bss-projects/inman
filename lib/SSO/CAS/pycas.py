...
def _parse_tag(string, tag):
    ...
    d_xmltree = string.split('\r\n')

    for line in d_xmltree:
     line = line.strip()
     obj = re.match('^<cas:user>([a-zA-Z]{1,}[0-9]{1,})', line)
     if obj:
      return obj.group(1)
...
def _make_pycas_cookie(val, domain, path, secure, expires=None, web_session=None):
    """Form cookie."""
    ...
    if not web_session:
    	os.environ['HTTP_COOKIE'] = pycascookie
    else :
    	web_session['pycas_cookie'] = pycascookie
    	web_session['login'] = val.split('.')[0]
    	web_session.save()
...
#    cas_url = cas_host + "/cas/login?service=" + service_url
    ...
    return TICKET_NONE, cas_url, None
...
def _get_cookie_expires(d_cookie_attr):
	for cookie_attr in d_cookie_attr:
		expires = re.match('^expires=([0-9]{1,})', cookie_attr)
		if expires:
			return expires.group(0)
...
def _decode_cookie(cookie_vals, lifetime=None):
    ...
    for cookie_val in cookie_vals:
        ...
        if cookie_val == "gateway":
            ...
            d_cookie_attr = cookie_val.split(';')
            #  Verify hash
            if SIGNER.unsign(d_cookie_attr[0]):
                #  Check lifetime
                lifetime = _get_cookie_expires(d_cookie_attr)
                if lifetime:
    ...
    if COOKIE_AUTH in cookie_attrs:
        return COOKIE_AUTH, d_cookie_attr[0].split('.')[0]
...
#    cas_validate = cas_host + "/websso/validate?ticket=" + ticket + "&service=" + service_url
    cas_validate = cas_host + "/validate?ticket=" + ticket + "&service=" + service_url
    context = ssl._create_unverified_context()
    f_validate = urllib.urlopen(cas_validate, context=context)
...
def _validate_cas_2(cas_host, service_url, ticket, opt):
    ...
    """
    #  Second Call to CAS server: Ticket found, verify it.
#    cas_validate = cas_host + "/websso/serviceValidate?ticket=" + ticket + "&service=" + service_url
...
def _get_cookies(web_session):
    ...
    try:
    	if not web_session:
        	...
        	cookie_pairs = web_session['pycas_cookie'].split()
...
def _get_cookie_status(web_session):
    """Check pycas cookie."""
    cookies = _get_cookies(web_session)
...
def _do_redirect_logout(cas_host, service_url):

    """Send redirect to client.  This function does not return, i.e. it teminates this script."""
    cas_url = cas_host + "/logout?service=" + service_url

    return TICKET_NONE, cas_url, None

def logout(cas_host, service_url, web_session=None, lifetime=None, secure=True, protocol=2, path="/", opt=""):

	if web_session:
		web_session.delete()
	return _do_redirect_logout(cas_host, service_url)

def login(cas_host, service_url, web_session=None, lifetime=None, secure=True, protocol=2, path="/", opt=""):
    """
    ...
    cookie_status, cookieid = _get_cookie_status(web_session)
    ...
    if ticket_status == TICKET_OK:
        ...
        return CAS_OK, ticketid, _make_pycas_cookie(cookie_val, domain, path, secure, web_session=web_session)
    ...
    if opt == "gateway":
        if cookie_status == COOKIE_GATEWAY:
            ...
            return CAS_GATEWAY, "", _make_pycas_cookie("", domain, path, secure, past_date, web_session=web_session)

    #  Do redirect
    return _do_redirect(cas_host, service_url, opt, secure)

#-----------------------------------------------------------------------
# Exemple de cookie retourne par le CAS et stocke
#-----------------------------------------------------------------------
